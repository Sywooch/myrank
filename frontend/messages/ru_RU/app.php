<?php

return [
    'REQUESTED_PAGE_WAS_NOT_FOUND'=>'Запрошенная страница не найдена',
    'MYRANK_SLOGAN'=>'Оцените надежность, честность и профессионализм	потенциального партнера или контрагента.',
    'FIND'=>'Найти',
    'REGION'=>'Регион',
    'SEARCH'=>'Поиск',
    'SELECT_YOUR_PROFILE'=>'Выберите ваш профиль',
    'ENTER_NAME_OR_SPECIALIST'=>'Введите имя или специалиста',
    'CATEGORY'=>'Категория',
    'RATING' => 'Рейтинг',
    'CHOOSE_WHO_YOU_ARE'=>'Выберите кто вы',
    'COMPANY'=>'Компания',
    'PERSON'=>'Особа',
    'IAM_USER'=>'Я пользователь',
    'WEARE_COMPANY'=>'Мы компания',
    'REGISTER_OR_SIGN_THROUGH_SOCIAL'=>'Зарегистрируйтесь или войдите через социальные сети',
    'REGISTRATION'=>'Регистрация',
    'LOGIN'=>'Вход',
    'REMEMBER_PASSWORD' => 'Забыли пароль?',
    'REQUEST_PASSWORD_RESET' => 'Востановление пароля',
    'RESET_PASSWORD' => 'Сменить пароль',
    'PLEASE_CHOOSE_YOUR_NEW_PASSWORD' => 'Пожалуйста, выберите новый пароль',
    'YOUR_COUNTRY'=>'Ваша страна',
    'WELCOME_TO_MYRANK'=>'Добро пожаловать в MyRank!',
    'AUTHORIZE'=>'Авторизоваться',
    'AUTHORIZATION'=>'Авторизация',
    'EXIT'=>'Выход',
    'ADDRESS'=>'Адрес',
    'MENU'=>'Меню',
    'PROFILE_AND_SERVICES'=>'Профиль и сервисы',
    'ENTER'=>'Войти',
    'LOGOUT'=>'Выйти',
    'SEND' => 'Отправить',
    'SAVE' => 'Сохранить',
    'REGISTER'=>'Зарегистрироваться',
    'RESTORE_ACCESS'=>'Восстановить доступ',
    'RECOMMENDATIONS_EXECUTOR'=>'Рекомендации Исполнителям',
    'RECOMMENDATIONS_CUSTOMER'=>'Рекомендации Заказчикам',
    'SITE_SEARCH'=>'Поиск по сайту',
    'PAID_OPTIONS_IN_PROJECTS'=>'Платные опции в проектах',
    'SITE_ADS'=>'Реклама на сайте',
    'WE_ARE_IN_SOCIAL_NETWORKS'=>'мы в соцсетях',
    'ADD_TO_GROUPS_AND_WATCH_ALL_THE_LATEST_NEWS_AND_ANNOUNCEMENTS'=>'Добавляйтесь в группы и следите за всеми свежими новостями и объявлениями!',
    'MYRANK_LTD_ALL_RIGHTS_RESERVED'=>'ООО "Майранк". Все права защищены.',
    'TOP_CATEGORIES'=>'Профессиональные направления',
    'LITTLE_ABOUT_THE_PROJECT'=>'Немного о проекте',
    'LITTLE_ABOUT_THE_PROJECT_TEXT'=>'<p>Сервис MyRank позволяет оценивать разнообразные качества пользователей по 10-балльной шкале и выводить общую среднюю оценку их рейтинга.</p><p>При оценивании пользователя по какому либо критерию, оценивающий имеет возможность ввести свой отзыв, пояснив текстом, почему он поставил ту или иную оценку.</p><p>По общей и средним оценкам строятся полярные диаграммы по категориям и критериям оценки. В идеальном случае, диаграмма должна представлять собой круг – колесо, которое свободно и ровно катится по жизни.</p><p>Отсюда вытекает миссия сервиса MyRank: помогать людям контролировать свои приоритеты и «выравнивать колесо» своей жизни.</p>',
    'NEWS'=>'Новости',
    'HOW_SERVICE_WORK'=>'Как работает сервис',
    'HOW_SERVICE_WORK_TEXT_1'=>'Регистрация Пользователя/Компании.',
    'HOW_SERVICE_WORK_TEXT_2'=>'Заполните информацию о профиле, чтобы получить максимально высокий бал.',
    'HOW_SERVICE_WORK_TEXT_3'=>'Оценивайте партнеров, коллег и знакомых, оставляйте о них отзывы.',
    'HOW_SERVICE_WORK_TEXT_4'=>'Повышайте рейтинг профиля, получайте отзывы.',
    'HOW_SERVICE_WORK_TEXT_5'=>'Высокий рейтинг - карьерный рост.',
    'HOW_SERVICE_WORK_TEXT_6'=>'Ищите партнеров для своего бизнеса.',
    'REGISTER_AND_GET_FULL_ACCESS_TO_ALL_FEATURES'=>'Зарегистрируйтесь и получите <span>полный доступ ко всем возможностям</span>',
    'INCORRECT_LOGIN_OR_PASSWORD'=>'Неверный логин или пароль',
    'BEST_RATING'=>'Лучший рейтинг',
    'BEST_RATING_TEXT'=>'ТОП пользователей с максимальным рейтингом.',
    'ALL_CATEGORIES' => 'Все категории',
    'ARTICLES_EMPTY_LIST'=>'Список пуст или запрошенная категория статей отсутствует',
    'ARTICLES_SHOWING'=>'Показано',
    'ARTICLES_SHOWING_2'=>'из',
    'ADDRESS_INFO'=>'ООО "Майранк", Офис 5, Бизнес-центр "Нипсэнд Триангл", 1, Бартон Роуд, Шеффилд, Южный Йоркшир, S3 8BW, Великобритания',
    'LAST_ADDED_USERS'=>'Последние добавленные пользователи',
    'TOP_USERS_WITH_HIGHEST_RATING'=>'ТОП пользователей с максимальным рейтингом.',
    'READ_ALSO'=>'Читайте также',
    'RECENT_MATERIALS'=>'Последние материалы',
    'NO_ARTICLES'=>'Новости отсутствуют',
    'FILE_NOT_SAVED'=>'Файл не сохранен',
    'FILE_NOT_UPLOADED' => 'Файл не загружен',
    'THANK_YOU_FOR_CONTACTING_US'=>'Благодарим Вас за обращение к нам. Мы ответим вам как можно скорее.',
    'THERE_WAS_ERROR_SENDING_EMAIL'=>'При отправке электронной почты произошла ошибка.',
    'CHECK_YOUR_EMAIL_FOR_FURTHER_INSTRUCTIONS'=>'Проверьте свою электронную почту для получения дальнейших инструкций.',
    'SORRY_WE_ARE_UNABLE_TO_RESET_PASSWORD_FOR_EMAIL_PROVIDED'=>'Извините, мы не можем сбросить пароль для электронной почты.',
    'NEW_PASSWORD_WAS_SAVED'=>'Новый пароль сохранен.',
    'PASSWORD_AND_REPEAT_DO_NOT_MATCH'=>'Пароль и повтор пароля не совпадают',
    'IN_ORDER_TO_USE_ALL_SERVICES_YOU_MUST_AUTHORIZE'=>'Для того чтоб пользоваться всеми сервисами сайта, надо',
    'REGISTRATION_STEP_1_OF_2'=>'Регистрация - Шаг 1<span> из 2</span>',
    'REGISTRATION_STEP_2_OF_2'=>'Регистрация - Шаг 2 <span> из 2 </span>',
    'YOUR_SCORE_HAS_BEEN_SAVED'=>'Ваша оценка сохранена',
    'GIVE_FEEDBACK'=>'Оставить отзыв',
    'EDITING_COMPANY'=>'Редактирование компании',
    'IN_TRUSTED'=>'В ДОВЕРЕННЫЕ',
    'IN_TRUSTED_SMALL'=>'В доверенные',
    'TRUSTED'=>'ДОВЕРЕННЫЙ',
    'TRUSTED_SMALL'=>'Доверенный',
    'LATEST_RATINGS'=>'Последние оценки',
    'USER_RATING'=>'Оценка пользователя',
    'TRUSTED_PERSONS'=>'Доверенные лица',
    'UNABLE_TO_SAVE_client_ACCOUNT_errors' => 'Не удалось сохранить учетную запись {client}: {errors}',
    'UNABLE_TO_SAVE_USER_errors'=>'Не удалось сохранить пользователя: {errors}',
    'LINKED_client_ACCOUNT'=>'Учетная запись {client} подключена.',
    'UNABLE_TO_LINK_client_ACCOUNT_errors'=>'Невозможно подключить учетную запись {client}: {errors}',
    'UNABLE_TO_LINK_client_ACCOUNT_THERE_IS_ANOTHER_USER_USING_IT'=>'Невозможно подключить учетную запись {client}. Есть другой пользователь использующий её.',
    'COUNTRY'=>'Страна',
    'NAME'=>'Имя',
    'SURNAME'=>'Фамилия',
    'CITY'=>'Город',
    'SPECIALIZATION'=>'Специализация',
    'LET_PEOPLE_KNOW_WHAT_YOU_ARE_DOING'=>'Позвольте людям узнать чем вы занимаетесь',
    'PASSWORD'=>'Пароль',
    'CONFIRM_PASSWORD'=>'Повторите пароль',
    'PLACE_OF_WORK_AT_THE_MOMENT'=>'Место работы на данный момент',
    'POSITION'=>'Должность',
    'PHONE_NUMBER'=>'Номер телефона',
    'PERSONAL_INFORMATION'=>'Информация о себе',
    'TELL_A_LITTLE_ABOUT_YOURSELF'=>'Расскажите немного о себе',
    'TELL_US_ABOUT_YOURSELF_NO_MORE_THAN_500_CHARACTERS'=>'Расскажите о себе. Не больше 500 символов.',
    'EXAMPLE_FIRSTNAME'=>'Михаил',
    'EXAMPLE_SURNAME'=>'Иванов',
    'EXAMPLE_EMAIL'=>'example@domain.com',
    'EXAMPLE_COMPANY_NAME'=>'ООО АстаМ',
    'EXAMPLE_COMPANY_POST'=>'СЕО',
    'EXAMPLE_PHONE'=>'0 44 XXX XX XX',
    'EXAMPLE_DATE'=>'20.12.1986',
    'EXAMPLE_DIRECTOR'=>'ФИО',
    'EXAMPLE_CONTACT_PERSON'=>'ФИО, должность',
    'COMPANY_NAME'=>'Название компании',
    'THE_NUMBER_OF_EMPLOYEES'=>'Количество сотрудников',
    'DATE_OF_REGISTRATION_OF_THE_COMPANY'=>'Дата регистрации компании',
    'ANNUAL_TURNOVER'=>'Годовой оборот, РУБ',
    'DIRECTOR'=>'Директор',
    'CONTACT_PERSON'=>'Контактное лицо',
    'INFORMATION_ABOUT_THE_COMPANY'=>'Информация о компании',
    'TELL_A_LITTLE_ABOUT_COMPANY'=>'Расскажите немного о компании',
    'TELL_US_ABOUT_COMPANY_NO_MORE_THAN_500_CHARACTERS'=>'Расскажите о компании. Не больше 500 символов',

    // MENU
    'HOME'=>'Главная',
    'ABOUT'=>'О сервисе',
    'ARTICLES'=>'Новости',
    'BALANCE'=>'Баланс',
    'HELP'=>'Помощь',
    'CONTACTS'=>'Контакты',
    'LEGALINFO'=>'Условия & защита',
    'PRIVACY_POLICY' => 'Политика конфиденциальности',

    'FILEUPLOAD'=>'Выберите файл ...',
    'MARK_MINE'=>'Моя оценка',
    'MARK'=>'Оценка',
    'RATING_AVERAGE'=>'Средняя оценка',
    'VIEW_ALL'=>'Посмотреть всех',
    'MARKS_DIAGRAM'=>'Диаграмма оценок',
    'RATING_SAVE'=>'Сохранить оценку',
    'REPLY'=>'Ответить',
    'COMPLAIN'=>'Пожаловаться',
    'MORE'=>'БОЛЬШЕ',
    'NO_REVIEWS'=>'Отзывов нет',
    'NO_REVIEWS_YOU_CAN_SHARE_USEFUL_INFORMATION_ABOUT_THIS_PERSON'=>'Отзывов нет, вы можете поделиться полезной информацией об этом человеке.',
    'CANCEL'=>'Отменить',
    'ENTER_TO_LEAVE_MARKS_FOR_USERS'=>'Войдите, чтобы оставлять оценки пользователям',
    'COMPLIANT_SUBMITTED'=>'Жалоба отправлена',
    'PORTFOLIO'=>'Портфолио',
    'UPLOAD_A_NEW_PROJECT_PHOTO'=>'Загрузите новое фото проекта',
    'PROJECT_NAME'=>'Название проекта',
    'PROJECT_DESCRIPTION'=>'Описание проекта',
    'NO_MORE_THAN_500_CHARACTERS'=>'Не больше 500 символов.',
    'PHOTO_DID_NOT_LOAD_PLEASE_CONSULT_ADMINISTRATOR'=>'Фото не загрузилось, пожалуйста обратитесь к администратору',
    'CHECK_CORRECTNESS_OF_THE_DATA_FILLING_FILL_IN_THE_MISSING_INFORMATION'=>'Проверьте правильность заполнения данных, заполните недостающую информацию',
    'CONFIGURE_THE_DISPLAY_OF_MARKS'=>'Настроить оценки',
    'YOUR_REQUEST_HAS_BEEN_SENT_AND_IS_WAITING_FOR_CONFIRMATION_BY_THE_USER'=>'Ваш запрос отправлен и ждет подтверждения пользователем',
    'FOUND'=>'Найдено',
    'ITEMS'=>'Пользователи/Компании',
    'VIEW_LIST_10'=>'По 10 шт',
    'VIEW_LIST_20'=>'По 20 шт',
    'USER_PROFILE'=>'Профиль пользователя',
    'COMPANY_PROFILE' => 'Профиль компании',
    'TRUSTED_WHOM'=>'Доверенных',
    'MARKS_WHOM'=>'Оценок',
    'REVIEWS_WHOM'=>'Отзывов',
    'MINIMIZE'=>'Свернуть',
    'UPLOAD_AVATAR'=>'Загрузить аватар',
    'UPLOAD_A_NEW_PHOTO'=>'Загрузите новое фото',
    'IAM'=>'Я',
    'TESTIMONIALS_TEXT'=>'Текст отзыва',
    'WRITE_A_FEW_WORDS'=>'Напишите несколько слов',
    'PROFILE_EDIT'=>'Редактировать профиль',
    'CONFIGURE'=>'Настроить',
    'TESTIMONIALS_EDIT'=>'Редактировать отзыв',
    'TESTIMONIALS'=>'Отзывы',

    // Models Begin
    'ID'=>'ИД',
    'USER_ID'=>'Пользователь ИД',
    'CATEGORY_ID'=>'Категория ИД',
    'CATEGORY_NAME'=>'Наименование категории',
    'VALUE'=>'Значение',
    'USER_FULL_NAME'=>'Полное имя пользователя',
    'NO_USER'=>'Нет пользователя',
    'ACCOUNT_ID'=>'Аккаунт ИД',
    'ROLE_USER_ID'=>'Роль пользователя ИД',
    'ROLE_ACCESS_ID'=>'Роль доступа ИД',
    'PAGE'=>'Страница',
    'EMAIL'=>'Email',
    'BODY' => 'Сообщение',
    'SUBMIT' => 'Отправить',
    'PASSWORD_HASH'=>'Хэш пароля',
    'PASSWORD_SALT'=>'Соль пароля',
    'PASSWORD_RESET_CODE'=>'Код сброса пароля',
    'PASSWORD_RESET_CODE_EXPIRES'=>'Срок действия кода сброса пароля',
    'USER_AUTH_TOKEN'=>'Токен авторизации пользователя',
    'AUTH_KEY'=>'Авторизационный ключ',
    'ACTIVE'=>'Активный',
    'CREATE_DATE'=>'Дата создания',
    'STATUS'=>'Статус',
    'LOCALE'=>'Локаль',
    'TIMEZONE'=>'Часовой пояс',
    'ARTICLE_ID' => 'ИД Новости',
    'TITLE' => 'Заглавие',
    'ABRIDGMENT' => 'Сокращенный текст',
    'CONTENT' => 'Содержимое',
    'HEADER_TITLE' => 'Заголовок заглавия',
    'HEADER_IMAGE' => 'Заголовок изображение',
    'HEADER_IMAGE_SMALL' => 'Заголовок среднее изображение',
    'HEADER_IMAGE_SMALL_SQUARE' => 'Заголовок уменьшенное изображение',
    'ARTICLE_CATEGORY_ID' => 'Категория новости ИД',
    'ARTICLE_CATEGORY_NAME' => 'Наименование категории новости',
    'VIEWS' => 'Просмотры',
    'CREATE_TIME' => 'Время создания',
    'UPDATE_TIME' => 'Время обновления',
    'YES'=>'Да',
    'NO'=>'Нет',
    'SOURCE_ID'=>'ИД Источника',
    'SOURCE'=>'Источник',
    'CITY_ID'=>'ИД Города',
    'COUNTRY_ID'=>'ИД Страны',
    'REGION_ID'=>'ИД Области',
    'CITY_NAME'=>'Название города',
    'COUNTRY_NAME'=>'Название страны',
    'REGION_NAME'=>'Название области',
    'ALL'=>'Все',
    'PHONE'=>'Телефон',
    'SITE'=>'Сайт',
    'COMPANY_PHONE' =>  'Телефон компании',
    'PERSONS_QUANTITY' => 'Количество сотрудников',
    'COMPANY_DATA_REGISTRATION' => 'Дата регистрации компании',
    'COMPANY_ANNUAL_TURNOVER' => 'Ежегодный оборот компании',
    'DIRECTOR_FULLNAME' => 'Фио директора',
    'COMPANY_ABOUT' => 'Информация о компании',
    'NOT_LISTED'=>'Не указано',
    'PROFESSION_ID'=>'ИД профессии',
    'PROFESSION_TITLE'=>'Наименование профессии',
    'LIST'=>'Список',
    'VERIFICATION_CODE'=>'Код подтверждения',
    'COMPANY_OR_USER_TYPE' => 'Тип компания или пользователь',
    'COMPANY_OR_USER_TYPE_ID' => 'ИД компании или пользователя',
    'IMAGE_NAME' => 'Название изображения',
    'LOGS_TYPE'=>'Тип регистрационного журнала',
    'TEXT'=>'Текст',
    'CREATED'=>'Создано',
    'THERE_IS_NO_USER_WITH_SUCH_EMAIL'=>'Нет пользователя с такой почтой.',
    'PASSWORD_RESET_FOR'=>'Пароль сброшен для',
    'LASTWEEK'=>'Последняя неделя',
    'LASTMONTH'=>'Последний месяц',
    'PASSWORD_RESET_TOKEN_CANNOT_BE_BLANK'=>'Токен сброса пароля не может быть пустым.',
    'WRONG_PASSWORD_RESET_TOKEN'=>'Wrong password reset token.',
    'THIS_USERNAME_HAS_ALREADY_BEEN_TAKEN'=>'Это имя пользователя уже занято.',
    'THIS_EMAIL_ADDRESS_HAS_ALREADY_BEEN_TAKEN'=>'Этот адрес электронной почты уже занят.',
    'MARKS_ACCESS_ALL'=>'Всем',
    'MARKS_ACCESS_USER'=>'Пользователю',
    'MARKS_ACCESS_COMPANY'=>'Пользователям',
    'MARKS_ACCESS_FRONT_ALL'=>'Всем',
    'MARKS_ACCESS_FRONT_NONE'=>'Отключить',
    'MARKS_NAME' => 'Название оценки',
    'PARENT_ID' => 'Родительский ИД',
    'PARENT_NAME'=>'Имя родителя',
    'MARKS_ACCESS' => 'Кому показывать',
    'MARKS_TYPE' => 'Специализация',
    'WITHOUT_PARENT' => 'Без родителя',
    'ALIAS'=>'Псевдоним',
    'PUBLISHED'=>'Опубликовано',
    'TITLE_BROWSER'=>'Заглавие браузера',
    'META_KEYWORDS'=>'Мета-ключевые слова',
    'META_DESCRIPTION'=>'Метаописание',
    
    // Список должностей
    'WHO_FROM_TO_NONE' => 'Не определен',
    'WHO_FROM_TO_DIRECTOR' => 'Директор/Владелец компании',
    'WHO_FROM_TO_CHIEF' => 'Начальник',
    'WHO_FROM_TO_COLLEAGUE' => 'Коллега',
    'WHO_FROM_TO_CLOSE_RELATIVE' => 'Близкий родственник',
    'WHO_FROM_TO_RELATIVE' => 'Родственник',
    'WHO_FROM_TO_FRIEND' => 'Друг',
    'WHO_FROM_TO_FAMILIAR' => 'Знакомый',
    'WHO_FROM_TO_SLAVE' => 'Подчинённый',
    'WHO_FROM_TO_UNKNOW' => 'Не знаком',
    'WHO_FROM_TO_ANONIM' => 'Аноним',
    'USER_FROM' => 'От пользователя',
    'USER_TO' => 'К пользователю',
    'SMILE' =>  'Улыбка',
    'PARENT_TEXT' => 'Текст родителя',
    'USER_FROM_FULLNAME'=> 'От пользователя полное имя',
    'USER_TO_FULLNAME'=> 'К пользователю полное имя',
    'CONTACT_ID' => 'ИД контакта',
    'COMPANY_ID' => 'ИД компании',
    'PROFILE_VIEWS' => 'Просмотры профиля',
    'PROFILE_COMPANY' => 'Профиль компании',
    'IMAGE' => 'Изображение',
    'LAST_LOGIN' => 'Последний вход',
    'BIRTHDATE' => 'Дата рождения',
    'GENDER'=>'Пол',
    'PROFESSION_FIELD' => 'Специализация',
    'IS_NOT_IMPLEMENTED' => 'не реализован',
    'OBJ' => 'Объект',
    'OBJ_ID' => 'ИД объекта',
    'NAME_TITLE' => 'Название',
    'DATETIME' => 'Дата/время',
    'MARK_ID' => 'ИД оценки',
    'MARK_VAL' => 'Значение оценки',
    'MARK_NO' => 'Нет оценки',
    'DESCRIPTION' => 'Описание',
    'NOTIFICATION_TYPE' => 'Тип уведомления',
    'PROFESSION_NO' => 'Нет профессии',

    // Models End
    
    // 
    'CREATE_STRUCT' => 'Создание структуры',
    'COMPANY_STRUCT' => 'Структура компании',
    'PERS_NO_STRUCT' => 'Сотрудники без структуры',
    'STRUCT' => 'Структура',
    'COMPANY_MARKS' => 'Оценки компании',
    'RATING_PERS' => 'Рейтинг сотрудников',
    'SPECIALISTS' => 'Специалисты',
    'RATING_COMP' => 'Рейтинг компании',
    'RATING_COMPANY' => 'Рейтинг компании',
    'MY_MARKS' => 'Мои оценки',
    'TRUSTED_COMPANY' => 'Доверенные компании',
    'MY_TRUSTEES' => 'Мои доверенные',
    'ALL_TRUSTEES' => 'Все доверенные',
    'RATING_FROM_COMPANY' => 'Рейтинг компаний',
    'RATING_FROM_COMPANY_IN' => 'Рейтинг компаний в ',
    'RATING_FROM_USERS' => 'Рейтинг пользователей',
    'ALL_RATING' => 'Весь рейтинг',
    'AVERAGE_RATING' => 'Средняя<br> оценка',
    'ALL_MARKS' => 'Все оценки',
    'TESTIMONIALS_COMPANY' => 'Отзывы компании',
    'MAIN_PAGE' => 'Главная',
    'MARKS_PRE_MESSAGE' => 'Для того чтоб сохранить оценку, войдите или зарегистрируйтесь',
    'RATING_USER' => 'Рейтинг пользователя',
    'CUSTOM_CONFIGURE_THE_DISPLAY_OF_MARKS' => 'Настраиваемая категория оценок',
    'NO_DATA' => 'Нет данных',
    'ACCEPT_LEGAL' => 'Согласен с пользовательским соглашением',
    'MARKS' => 'Оценки',
    'TRUSTEES' => 'Доверенные',
    'SELECT_UP_TO_TEN_CRITERIA' => 'Выберите до десяти критериев оценки',
    'MAIN_PROFESSION' => 'Основной вид деятельности',
    
    // STATUSES
    'CONFIRM' => 'Подтвердить',
    'REFUSE' => 'Отказать',
    'REMOVE' => 'Удалить',
    'SELF_ASSESSMENT' => 'Самооценка',
    'OTHER_ASSESSMENT' => 'Другие',
    
    
    // MENU
    'INFORMATION' => 'Информация',
    'MY_TESTIMONIAL' => 'Мои отзывы',
    'PERSONALS' => 'Сотрудники',

    
    // HELP FORM
    'PROBLEM' => 'Проблема (в кратце опишите проблему)',
    'PROBLEM_TYPE_VAL' => 'Тип проблемы',
    'QUESTION' => 'Вопрос',
    'PROBLEM_NONE' => 'Нет проблем',
    'PROBLEM_1' => 'Заполнение страницы компании',
    'PROBLEM_2' => 'Заполнение страницы пользователя',
    'PROBLEM_3' => 'Учётная запись, настройки и разрешения',
    'PROBLEM_4' => 'Реклама',
    'PROBLEM_5' => 'Вход в систему и пароль',
    'PROBLEM_6' => 'Конфиденциальность и злоупотребление',
    'PROBLEM_7' => 'Техническая поддержка',
    'PROBLEM_8' => 'Жалоба на клевету',
    'PROBLEM_9' => 'Другое',
    'HELP_FORM_SEND_MESSAGE' => 'Ваш вопрос был отправлен, в течение 3 суток он будет рассмотрен. Уведомление о решении вы получите на почту.',
    'CONTACT_FORM_SEND_MESSAGE' => 'Ваше сообщение успешно отправлено, в течение 3 суток оно будет рассмотрено. Ответ вы получите на почту.',
    'CONTACT_US_FORM' => 'Свяжитесь с нами',
];
